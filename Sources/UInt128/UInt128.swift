...
public struct UInt128 {
    ...
    public init?(_ source: String) {
        guard let result = Self._valueFromString(source) else {
            return nil
        }
        self = result
    }
...
extension UInt128: FixedWidthInteger {
    ...
    public func addingReportingOverflow(_ rhs: Self) -> (partialValue: Self, overflow: Bool) {
        ...
        return (partialValue: Self(upperBits: upperBits, lowerBits: lowerBits),
    ...
    public func multipliedFullWidth(by other: Self) -> (high: Self, low: Self.Magnitude) {
        ...
        return (high: Self(upperBits: upperUpperBits.truncatedValue, lowerBits: lowerUpperBits.truncatedValue),
                low: Self(upperBits: upperLowerBits.truncatedValue, lowerBits: lowerLowerBits.truncatedValue))
    }
    ...
    /// Returns the bit stored at the given position for the provided double width Self input.
    ///
...
extension UInt128: BinaryInteger {
    ...
    public static func &=(lhs: inout Self, rhs: Self) {
        ...
        lhs = Self(upperBits: upperBits, lowerBits: lowerBits)
    }
    ...
    public static func &>>=(lhs: inout Self, rhs: Self) {
        ...
        case 64:
            ...
            lhs = Self(upperBits: 0, lowerBits: lhs.value.upperBits)
        default:
            let lowerBits = lhs.value.upperBits >> (shiftWidth - 64)
            lhs = Self(upperBits: 0, lowerBits: lowerBits)
        }
    }
    ...
    public static func &<<=(lhs: inout Self, rhs: Self) {
        ...
        case 64:
            ...
            lhs = Self(upperBits: lhs.value.lowerBits, lowerBits: 0)
        default:
            let upperBits = lhs.value.lowerBits << (shiftWidth - 64)
            lhs = Self(upperBits: upperBits, lowerBits: 0)
        }
    }
}
...
extension UInt128: CustomStringConvertible {
    ...
    internal func _valueToString(radix: Int = 10, uppercase: Bool = true) -> String {
        ...
        var divmodResult = (quotient: self, remainder: Self(0))
        ...
        repeat {
            divmodResult = divmodResult.quotient.quotientAndRemainder(dividingBy: Self(radix))
...
extension UInt128 {
    ...
    public static func fromUnparsedString(_ source: String) throws -> Self {
    ...
    public init(stringLiteral value: StringLiteralType) {
        ...
        if let result = Self._valueFromString(value) {
            self = result
        }
    }
}
...
extension UInt128 {
    ...
    internal static func _valueFromString(_ value: String) -> Self? {
        let radix = Self._determineRadixFromString(value)
        let inputString = radix == 10 ? value : String(value.dropFirst(2))
        guard inputString.isEmpty == false else {
            return nil
        }

        return Self(inputString, radix: radix)
    }
